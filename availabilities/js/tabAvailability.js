$(function () {
  ...
  function fillDataTable() {
    var dataTable = $("#tab_availability").DataTable({
      dom: "<tp>",
      serverSide: true,
      searching: true,
      destroy: true,
      ...
      columnDefs: [
        ...
        },
        {
          ...
          createdCell: function (td, cellData, rowData, row, col) {
            $(td).html(
              `<i class="bi bi-box-arrow-up-left" onclick="viewDetails(${rowData[10]}, '${rowData[1]}')" data-bs-toggle="modal" data-bs-target="#exampleModal"></i>`
            );
          },
        },
        {
          ...
          data: null,
          createdCell: function (td, cellData, rowData, row, col) {
            let data = { id: rowData[10], status: rowData[2] };
            $(td).html(
      ...
      drawCallback: function (settings) {
        ...
        $("#name").on("change", function () {
          $("#tab_availability").DataTable().ajax.reload(); // Refresh table to filter records based on the selected Name
        });
      },
    });
  }
  ...
  $("#day").on("change", function () {
    $("#tab_availability").DataTable().ajax.reload();
  });
